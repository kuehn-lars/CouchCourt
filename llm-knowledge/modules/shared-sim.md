...
for why, and [[architecture]] for where it sits. **How a swing becomes a shot
is [[0015-contact-model]]; read it before changing anything below.**
...
| `src/shared/sim/shot.ts` | Timing windows, `timingOf`, and the launch solvers `groundstroke` / `serveShot`. The feel core |
| `src/shared/sim/serve.ts` | The toss, `TOSS_APEX`, deuce/ad positions and serve aim |
| `src/shared/sim/players.ts` | `predictStrike` — where and when a player meets the ball; stance, recovery, `movePlayer` |
...
rally.ts  ── imports ──▶  ball.ts     (stepBall)
   ...
   │                      shot.ts     (groundstroke, serveShot, timingOf, windows)
...
bot.ts    ── imports ──▶  rally.ts (MatchState type), serve.ts (TOSS_APEX), shot.ts (windows)
```
...
2. Apply every `RallyInput`, each at its **swing time** (arrival − `lag`):
   toss / serve, arm, strike (rewinding), revise, or ignore.
...
a rally ball commits once to a timing error aimed at the side of the court the
opponent is not on. Skill is an **error rate** (`ERROR_RATE × (1 − skill)`,
sprayed out wide) plus small placement noise, from a hashed per-side sequence
so a solo match replays exactly. See [[2026-09-22-contact-model-feel]] for why
it is a rate and not a smear.

Two skill-1 bots still rally indefinitely; 0.85 v 0.6 and 0.7 v 0.7 finish sets.
...
| **Balance** — tuned against a simulated human | `src/shared/sim/shot.ts`, `src/shared/sim/players.ts`, `REACTION` in `src/shared/sim/rally.ts`, `src/shared/sim/bot.ts` | Re-run the harness in [[2026-09-22-contact-model-feel]]; they interact |
